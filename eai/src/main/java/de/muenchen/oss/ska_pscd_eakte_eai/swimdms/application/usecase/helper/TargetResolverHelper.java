package de.muenchen.oss.ska_pscd_eakte_eai.swimdms.application.usecase.helper;

import de.muenchen.oss.ska_pscd_eakte_eai.swimdms.application.port.out.DmsOutPort;
...
public class TargetResolverHelper {DmsProperties dmsProperties;
    ...
    public UseCaseType resolveTypeFromMetadataFile(final Metadata metadata) throws MetadataException {
        ...
        final String metadataDmsTarget = indexFields.get(dmsProperties.getMetadataDmsTargetKey());
        if (StringUtils.isBlank(metadataDmsTarget)) {
            throw new MetadataException("DMS target key '%s' not found in metadata file or empty".formatted(dmsProperties.getMetadataDmsTargetKey()));
        }
        ...
        try {
            ...
            throw new MetadataException(
                    String.format("DMS target type via metadata file: Unexpected %s value: %s", dmsProperties.getMetadataDmsTargetKey(), metadataDmsTarget),
                    e);
        }
    }
}
